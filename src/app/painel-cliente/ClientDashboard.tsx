    ...
    fetch("/api/dashboard")
      ...
      .then(data => {
        if (data.isClient && data.appointments) {
          const sorted = data.appointments.sort((a: {date: string}, b: {date: string}) => new Date(b.date).getTime() - new Date(a.date).getTime());
                      ...
                      <div className="flex gap-4 items-center">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-gray-500">
                            {apt.barber ? `Com ${apt.barber.name}` : `Status: ${apt.status === "COMPLETED" ? "Concluído" : apt.status === "CANCELLED" ? "Cancelado" : "Pendente"}`}
                          </p>
                        </div>
                      </div>
